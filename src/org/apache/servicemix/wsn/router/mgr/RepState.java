...
public class RepState extends AState {
	...
	public void join() {
		...
		if (mng_ != null && mng_.isOK) {
			...
			PolicyDB pdb = mng_.policys;
			if(pdb != null) {
				setPolicyTime(pdb.time);
			...
			if (mng_.groups != null && !mng_.groups.isEmpty()) {
				...
				if (!mng_.groups.values().isEmpty()) {
					...
					for (int i = 0; i < times; i++) {
						...
						System.out.println("ask lsdb and policys from :" + g.name);
						if (g != null) {
							try {
								...
								System.out.println("Group "+g.name+" refused to offer LSDB");
								log.warn(e);
							} catch (ClassNotFoundException e) {
								e.printStackTrace();
								log.warn(e);
	...
	public boolean addLSAToLSDB(LSA lsa) {
		...
		if (lsdb.containsKey(lsa.originator)) {
			...
			if (old.seqNum >= lsa.seqNum)
				return false;
			System.out.println("add lsa from " + lsa.originator + "to lsdb");
			if (lsa.syn == 1) { // 同步LSA
				...
				if (!lsa.cancelTopics.isEmpty()) {
					...
					changedTopics.addAll(mgr.DeleteSubsByGroup(lsa.originator,
							lsa.cancelTopics));
				} // if
		...
		if (!lsa.subsTopics.isEmpty()) {
			for (String t : mgr.AddSubsByGroup(lsa.originator, lsa.subsTopics)) {
	...
	public void sendSbp(Object msg) {
		...
		// if cacheLSA is null before add the subscriptions, then set the timer
		// for sending lsa after the add action
		if (needInit) {
			Timer timer = new Timer();
			timer.schedule(new SendTask(), 2000);
		}
	...
	class SendTask extends TimerTask {
		...
		public void run() {
			...
			if (cacheLSA != null) {
				...
				spreadInLocalGroup(cacheLSA);

				// send the cacheLSA to the neighbors
				sendObjectToNeighbors(cacheLSA);

				addLSAToLSDB(cacheLSA);
	...
	public void lost(String indicator) {
		...
		if (fellows.containsKey(indicator)) {
			...
			timerForLost.schedule(lt, RtMgr.threshold);
		}
	}
		...
		new Thread() {
			public void run() {
				...
				try {
					...
					System.out.println("TCP socket with "+s.getInetAddress()+" cannot be continued ");
					log.warn(e);
					try {
						oos.close();
						ois.close();
	...
	private boolean processKindTcpMsg(ObjectInputStream ois,
		...
		if (msg instanceof MsgJoinGroup) {
			...
			MsgNewRep mnr = (MsgNewRep) msg;
			mnr.processRepMsg(ois, oos, s, mnr);
			return false;
			...
			mgl.processRepMsg(ois, oos, s, mgl);
			return false;
	...
	public void synLSA() {
		...
		this.addTopicsToLSA(lsa, topics, 0);
		this.addLSAToLSDB(lsa);

		this.sendObjectToNeighbors(lsa);
		this.spreadInLocalGroup(lsa);
	}
}
