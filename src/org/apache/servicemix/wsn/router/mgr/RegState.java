		...
		if (lsdb.containsKey(lsa.originator)) {
			...
			if (!lsa.lostGroup.isEmpty()) {
				for (String lost : lsa.lostGroup) {
					...
					if(lsa.originator.equals(groupName) && lsa.originator.equals(groupName) && neighbors.contains(lost)) {
...
/*	@Override
	public boolean addLSAToLSDB(LSA lsa) {
		if (lsdb.containsKey(lsa.originator)) {
			...
			if (!lsa.lostGroup.isEmpty()) {
				for (String lost : lsa.lostGroup) {
					this.deleteLineFromLSDB(lost, lsa.originator);
					if(lsa.originator.equals(groupName) && neighbors.contains(lost)) {
						neighbors.remove(lost);
					}
				}
			}
	...
	public void lost(String indicator) {
		...
		if (tmp == null) {
			...
			if (topics.size() != 0) {
				mgr.DeleteSubsByGroup(groupName, topics);
			}
