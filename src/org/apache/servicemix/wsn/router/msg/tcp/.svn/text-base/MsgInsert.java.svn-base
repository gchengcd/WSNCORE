	...
	public void processRepMsg(ObjectInputStream ois,
		...
		if (insertOK(mi.name)) {
			...
			if (mi.needInit) {
				...
				for (String n : li)
					if (state.getGroupMap().get(n) != null && !n.equals(mi.name)) {
						try {
							p = new DatagramPacket(
									buf,
									buf.length,
									InetAddress.getByName(state.getGroupMap().get(n).addr),
									state.getGroupMap().get(n).uPort);
							ds.send(p);
						} catch (IOException e) {
							e.printStackTrace();
						}
			...
			if (state.getGroupMap().keySet().contains(mi.name)) {
				...
				state.getGroupMap().put(g.name, g);
			}
